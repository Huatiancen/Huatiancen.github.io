...
核心模式：针对采用微服务系统在特定场景下的特定问题所使用的架构解决方案集合

- 服务注册与发现：服务消费者获取服务提供者位置的机制，以实现二者之间的解耦；服务启动时注册，服务调用时发现
  - 平台级服务发现：由平台或基础设施维护服务实例和地址，调用方通过平台能力定位服务，典型场景是 Kubernetes Service / DNS
  - 应用层服务发现 / 注册：服务实例启动时把自身地址注册到注册中心，消费者调用前查询可用实例，典型场景是 ZooKeeper、Eureka、Nacos
  - 价值：服务实例数量、位置和状态会动态变化，服务发现把“调用哪个地址”的问题从业务代码中解耦出来
- API Gateway：所有客户端的单一入口点，针对不同客户端提供不同 API，解决微服务架构中客户端如何访问各项服务的问题
  - 价值：客户端不需要知道内部服务拆分细节，网关可以集中处理路由、认证、限流、协议转换、聚合响应和不同终端适配
- 熔断器 / Circuit Breaker：避免任何服务或网络问题导致级联故障，并允许后续尝试再次调用
...
微服务数量多、调用链长、故障可能跨服务传播，所以必须把运行状态显式观测出来。

- Logging 日志：记录服务内部事件、异常、请求上下文，回答“发生了什么”。
- Metrics 指标：记录 QPS、延迟、错误率、资源使用率，回答“系统状态是否异常”。
- Tracing 链路追踪：串起一次请求经过的多个服务，回答“慢在哪里、错在哪里”。
- Health check 健康检查：让平台判断实例是否可用，并配合自动重启、摘除或扩容。
- Alerting 告警：当指标或日志满足异常条件时通知运维或开发人员。

考试答题句：微服务把复杂度从单体内部转移到分布式协作和运维层，因此不仅要会拆服务，还要通过日志、指标、链路追踪和健康检查支撑故障定位、恢复和扩缩容。
