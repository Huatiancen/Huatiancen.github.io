...
核心思想：先让机器稳定执行，把程序与数据组成作业，统一送入系统执行；执行围绕批处理与调度组织，强调吞吐与稳定。

迁移原因：硬件昂贵、交互几乎不存在，软件首先服务于“让机器跑起来”和提高昂贵算力利用率。当任务从“算一次”走向“重复算、稳定算、规模算”时，需要把重复任务写进程序，把执行秩序写进流程。
...
核心思想：通过“强中心 + 弱终端”换取一致性、安全性与治理能力。主机承担事务处理、数据存储和权限控制，终端主要负责输入与显示。
...
核心思想：把富交互从中心系统迁移到桌面端。客户端负责 UI 与部分本地逻辑，服务器负责共享数据与部分公共逻辑。
...
- 优先保障：易用性、响应性、交互体验。
- 相对牺牲：可部署性、可维护性、兼容性、统一治理。

### 三层 / 分层

核心思想：通过职责分层控制变化影响，而不是只做客户端瘦身。表示层负责接入与界面呈现，业务层承载业务规则和流程控制，数据层负责持久化与数据访问。

迁移原因：C/S 的客户端安装、升级、兼容成本随着规模上升而爆炸；客户端直接或强耦合访问数据层，不利于统一治理；一个系统里混杂了展示、业务和数据处理。因此重心从“前后分工”进一步转向“服务端内部清晰分层”。

质量属性影响：

- 优先保障：可维护性、可修改性、安全性、可部署性。
- 相对牺牲：极致性能、跨系统协同灵活性、快速局部自治。

### SOA

核心思想：把“系统功能”提升为“可复用的服务能力”。能力通过服务契约暴露，ESB / 中间件负责路由、转换、编排与集成，治理体系负责版本、规范、注册、权限与生命周期。
...
核心思想：让系统边界、团队边界、发布边界尽量对齐。系统围绕相对独立的业务能力拆成多个小服务，每个服务由相对独立的团队负责开发、部署与运维，数据尽量跟随服务边界自治。
...
核心思想：事件驱动解决“如何协作”，云原生解决“如何运行”。生产者发布事实，消费者按需订阅并处理；消息 / 流平台承担传递、缓冲、削峰与异步解耦；云原生平台承担编排、扩缩容、恢复、配置与观测。
...
核心思想：传统系统继续定义主流程，AI 负责语义增能而不接管全流程。现有业务系统保留主流程与主数据，新增模型调用层、RAG、工具调用、审计与评估等组件。

迁移原因：用户开始直接用自然语言工作，传统软件擅长规则与流程，却不擅长开放式语义任务；企业希望先把 AI 接到现有系统上，而不是推翻旧系统。

质量属性影响：

- 优先保障：易用性、知识获取能力、自然语言交互能力、内容生成能力。
- 相对牺牲：可预测性、测试性、输出可控性、治理复杂度。

### 复习抓手

考试如果问架构演化，不要只背“出现顺序”，要按下面三步答：

```text
1. 旧架构在新约束下哪里失效。
2. 新架构用什么结构边界解决核心矛盾。
3. 新架构提升哪些质量属性，又牺牲哪些质量属性。
```
