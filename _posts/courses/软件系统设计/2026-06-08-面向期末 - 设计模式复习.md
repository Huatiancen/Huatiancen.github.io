...
- 软件实体应当对扩展开放，对修改关闭
- 即在不修改源代码的基础上扩展系统行为
- 可通过**可变性封装原则 （Principle of Encapsulation of Variation, EVP）**更具体地理解：找到系统变化点，并将变化点封装起来
...
- 别名：Static Factory Method Pattern。
...
- 核心意图：为接口中的所有方法提供默认空实现，让子类只覆盖自己关心的方法。
- 适用关键词：接口方法很多、子类只需要实现少数方法、不希望每个子类都写一堆空方法。
- 掌握重点：缺省适配器通常是一个抽象类，它实现目标接口并提供默认实现，具体子类继承它后按需重写。

<img src="https://hua-tian-cen.oss-cn-hangzhou.aliyuncs.com/img/%E6%88%AA%E5%B1%8F2026-06-15%2020.02.15.png" alt="截屏2026-06-15 20.02.15" style="zoom:50%;" />

双向适配器 Two-way Adapter：

- 核心意图：适配器同时持有 `Target` 和 `Adaptee` 的引用，使两边都可以通过适配器调用对方的方法。
- 适用关键词：两个已有接口都需要复用，并且希望两个方向都能互相适配。
- 掌握重点：普通适配器通常只把 `Adaptee` 适配成 `Target`；双向适配器可以在 `Target` 和 `Adaptee` 之间双向转换调用。

![截屏2026-06-15 20.01.57](https://hua-tian-cen.oss-cn-hangzhou.aliyuncs.com/img/%E6%88%AA%E5%B1%8F2026-06-15%2020.01.57.png)

### 组合模式 Composite Pattern

- 核心意图：将对象组合成树形结构表示整体-部分层次，使客户端一致处理单个对象和组合对象。
- 别名：**Part-Whole Pattern**
...
- 易错点：**透明组合（Transparent Composite Pattern）**把 `add/remove/getChild` 放在 `Component` 中，强调一致性；安全组合只放在 `Composite` 中，强调类型安全**

### 桥接模式 Bridge Pattern

- 核心意图：将抽象部分与实现部分分离，使二者可以独立变化。
- 别名：**Handle and Body Pattern** / **Interface Pattern**
...
- 单纯享元对象是可以直接共享的基本对象；复合享元对象由多个单纯享元对象组合而成，它本身通常不共享，但它内部包含的单纯享元可以共享。复合享元也实现 Flyweight 接口，因此客户端可以像使用普通享元一样使用复合享元
...
![截屏2026-06-09 10.19.54](https://hua-tian-cen.oss-cn-hangzhou.aliyuncs.com/img/%E6%88%AA%E5%B1%8F2026-06-09%2010.19.54.png)
...
- 缺点：具体命令类可能过多。
- 易错点：画类图时一定标清 `Command`、`ConcreteCommand`、`Invoker`、`Receiver`
- 宏命令是命令模式和组合模式联用，类图如下

<img src="https://hua-tian-cen.oss-cn-hangzhou.aliyuncs.com/img/%E6%88%AA%E5%B1%8F2026-06-22%2009.25.49.png" alt="截屏2026-06-22 09.25.49" style="zoom:50%;" />

### 观察者模式 Observer Pattern

- 核心意图：定义对象间一对多依赖关系，使一个对象状态变化时，所有依赖对象得到通知并自动更新。
- 别名 / 相关说法：Publish-Subscribe。
...
- 优点：可以实现表示层和数据逻辑层的分离；观察目标和观察者抽象耦合；支持广播通信；新增观察者方便，符合开闭原则。
...
- 优点：简化对象交互；同事类解耦；减少子类生成；简化同事类的设计和实现
...
- 在模板方法模式中，子类不显式调用父类的方法，而是通过覆盖父类的方法来实现某些具体的业务逻辑，父类控制对子类的调用，这种机制被称为好莱坞原则(Hollywood Principle）
